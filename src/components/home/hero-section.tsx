...
const HERO_VIDEO_FULL_HD = "https://videos.pexels.com/video-files/9339478/9339478-hd_1920_1080_24fps.mp4";
const HERO_VIDEO_HD = "https://videos.pexels.com/video-files/9339478/9339478-hd_1280_720_24fps.mp4";
const HERO_POSTER = "https://images.pexels.com/videos/9339478/pexels-photo-9339478.jpeg?auto=compress&fit=crop&w=1600";
...
export function HeroSection() {
  ...
  useEffect(() => {
    ...
    let frame = 0;
    const revealHero = () => { frame = window.requestAnimationFrame(() => setHeroCanAnimate(true)); };
    if (html.hasAttribute("data-urechem-intro-active")) {
      introObserver = new MutationObserver(() => {
        if (!html.hasAttribute("data-urechem-intro-active")) { introObserver?.disconnect(); revealHero(); }
      });
      introObserver.observe(html, { attributes: true, attributeFilter: ["data-urechem-intro-active"] });
    } else revealHero();
    return () => { introObserver?.disconnect(); window.cancelAnimationFrame(frame); };
  }, [shouldReduceMotion]);

  useEffect(() => {
    const html = document.documentElement;
    html.removeAttribute(HERO_COMPLETE_ATTRIBUTE);
    if (!heroCanAnimate) return () => html.removeAttribute(HERO_COMPLETE_ATTRIBUTE);
    const timer = window.setTimeout(() => {
      html.setAttribute(HERO_COMPLETE_ATTRIBUTE, "true");
      window.dispatchEvent(new Event(HERO_COMPLETE_EVENT));
    }, shouldReduceMotion ? 0 : HERO_SEQUENCE_DURATION_MS);
    return () => { window.clearTimeout(timer); html.removeAttribute(HERO_COMPLETE_ATTRIBUTE); };
  ...
  useEffect(() => {
    ...
    let introObserver: MutationObserver | null = null;
    const playVideo = () => {
      if (cancelled || document.hidden || !isInViewport || html.hasAttribute("data-urechem-intro-active")) return;
      void video.play().catch(() => { if (!cancelled) setVideoFailed(true); });
    };
    const schedulePlay = () => { window.cancelAnimationFrame(playFrame); playFrame = window.requestAnimationFrame(playVideo); };
    const viewportObserver = new IntersectionObserver(([entry]) => {
      isInViewport = Boolean(entry?.isIntersecting);
      if (isInViewport) schedulePlay(); else { window.cancelAnimationFrame(playFrame); video.pause(); }
    }, { threshold: 0 });
    viewportObserver.observe(section);
    if (html.hasAttribute("data-urechem-intro-active")) {
      introObserver = new MutationObserver(() => {
        if (!html.hasAttribute("data-urechem-intro-active")) { introObserver?.disconnect(); schedulePlay(); }
      });
      introObserver.observe(html, { attributes: true, attributeFilter: ["data-urechem-intro-active"] });
    } else schedulePlay();
    const handleVisibilityChange = () => {
      if (document.hidden) { window.cancelAnimationFrame(playFrame); video.pause(); }
      else if (isInViewport) schedulePlay();
    };
    ...
    <section className="minimal-hero relative isolate overflow-hidden border-b border-blue-100 bg-slate-100 md:min-h-[calc(100dvh-4.5rem)]" data-urechem-hero-section ref={sectionRef}>
      ...
      <Container className="flex min-h-[38rem] items-center py-16 sm:min-h-[42rem] md:min-h-[calc(100dvh-4.5rem)] md:py-24">
        <div className="relative z-10 max-w-2xl">
          <motion.h1 animate={animationState} aria-label="Intelligent chemistry for better polyurethane solutions." className="hero-title max-w-[20rem] text-[clamp(2.35rem,8.5vw,3rem)] font-semibold leading-[1.02] tracking-[-0.035em] sm:max-w-2xl sm:text-[clamp(3.25rem,4.6vw,4.5rem)] sm:leading-[0.98]" initial={shouldReduceMotion ? false : "hidden"} variants={titleContainerVariants}>
            {heroTitleWords.map((word, index) => <motion.span aria-hidden="true" className="mr-[0.22em] inline-block will-change-transform last:mr-0" key={`${word}-${index}`} variants={titleWordVariants}>{word}</motion.span>)}
          </motion.h1>
          <motion.p animate={animationState} className="hero-tagline mt-6 text-base font-semibold sm:text-lg" custom={0.92} initial={shouldReduceMotion ? false : "hidden"} variants={supportingCopyVariants}>We deliver what we promise.</motion.p>
          <motion.p animate={animationState} className="hero-description mt-3 max-w-lg text-sm leading-6 sm:text-base sm:leading-7" custom={1.08} initial={shouldReduceMotion ? false : "hidden"} variants={supportingCopyVariants}>Advanced polyurethane systems, specialty chemicals and technical support for real-world applications.</motion.p>
          <motion.div animate={animationState} className="hero-process mt-10 grid max-w-xl grid-cols-3 border-y border-white/25" initial={shouldReduceMotion ? false : "hidden"} variants={processContainerVariants}>
            {processSteps.map((step, index) => <motion.div className={`py-4 ${index > 0 ? "border-l border-white/20 pl-5 sm:pl-6" : "pr-4"}`} custom={index} key={step} variants={processStepVariants}>
              <span className="hero-step-number block text-[0.62rem] font-semibold tracking-[0.24em]">0{index + 1}</span>
              <span className="hero-step-label mt-1.5 block text-xs font-medium uppercase tracking-[0.16em] sm:text-sm">{step}</span>
            </motion.div>)}
          </motion.div>
        </div>
      </Container>
      <style jsx global>{`
        ...
        .minimal-hero .hero-title { color: #ffffff !important; text-shadow: 0 3px 18px rgba(2, 15, 28, 0.26); }
