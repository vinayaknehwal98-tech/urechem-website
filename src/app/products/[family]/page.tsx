...
const FLEXIBLE_SYSTEMS_BACKGROUND =
  "https://images.unsplash.com/photo-1653601983541-a70f6f1e715b?auto=format&fit=crop&fm=jpg&q=84&w=2400";
...
export default async function Page({ params }: { params: Promise<{ family: string }> }) {
  ...
  const hasComfortFoamBackground = productFamily.slug === "flexible-systems";

  return (
    <div className="relative isolate overflow-hidden">
      {hasComfortFoamBackground ? (
        <>
          <div
            aria-hidden="true"
            className="absolute inset-0 -z-20 bg-cover bg-[center_48%] md:bg-[center_55%]"
            style={{ backgroundImage: `url(${FLEXIBLE_SYSTEMS_BACKGROUND})` }}
          />
          <div
            aria-hidden="true"
            className="absolute inset-0 -z-10 bg-[linear-gradient(90deg,rgba(255,255,255,0.84)_0%,rgba(255,255,255,0.72)_48%,rgba(255,255,255,0.78)_100%)]"
          />
          <div
            aria-hidden="true"
            className="absolute inset-x-0 bottom-0 -z-10 h-64 bg-gradient-to-t from-white/80 via-white/35 to-transparent"
          />
        </>
      ) : null}
